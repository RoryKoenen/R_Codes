...
l<-as.integer(n*0.85) #take 85% of total curve to fit
...
y<-Xa_curve$data
f<-Xa_curve$fit
plot(x,y,main = "Xa_curve + 4th degree fit", pch="°", xlab = "time", ylab = "mOD", xlim = c(0,5), ylim = c(0,60))
lines(x,f, col="red")
...
y<-Xa_fit.df$res
f<-Xa_fit.df$val
ylim.1<-0.1*max(Xa_fit.df$res)
ylim.1b<- 0.1*min(Xa_fit.df$res)
xlim.1<-0.4*max(Xa_fit.df$data)
